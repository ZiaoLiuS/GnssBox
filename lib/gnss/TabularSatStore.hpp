...
#ifndef TABULAR_SAT_STORE_INCLUDE
#define TABULAR_SAT_STORE_INCLUDE
...
#pragma clang diagnostic pop
      ...
      try {
         ...
         if(satit == tables.end()) {
            InvalidRequest
               e("Satellite " + sat.toString() + " not found.");
            THROW(e);
         }
          ...
          const noexcept(false)
      {
         try {
            ...
            satit = tables.find(sat);
            if(satit == tables.end()) {
               InvalidRequest ir("Satellite " + sat.toString()
                   + " not found.");
               THROW(ir);
            }
   ...
   // end interface like that of XvtStore (also hasVelocity() and isPresent())
      ...
      /// Get a list (std::vector) of SatIDs present in the store
      std::set<SatID> getSatSet(void) const throw()
      {
         std::set<SatID> satSet;
         typename SatTable::const_iterator it;
         for(it=tables.begin(); it != tables.end(); ++it)
            if(it->second.size() > 0) satSet.insert(it->first);
         return satSet;
      }
...
#endif // TABULAR_SAT_STORE_INCLUDE
