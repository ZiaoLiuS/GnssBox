...
namespace gnssSpace
{
      ...
      noexcept(false)
   {
      ...
      {
         valid = false;
         THROW(InvalidParameter("Invalid temperature parameter."));
      }
      if (press < 0.0)
      {
         valid = false;
         THROW(InvalidParameter("Invalid pressure parameter."));
      }
      if (humid < 0.0 || humid > 100.0)
      {
         valid = false;
         THROW(InvalidParameter("Invalid humidity parameter."));
      }
      ...
      noexcept(false)
   {
      ...
      {
         valid = false;
         THROW(InvalidParameter("Invalid weather data"));
      }
   }
      ...
      noexcept(false)
   {
      ...
      {
         if(!validWeather)
            THROW(InvalidTropModel("Invalid GGH trop model: Weather"));
         if(!validHeights)
            THROW(InvalidTropModel("Invalid GGH trop model: Heights"));
         if(!validRxHeight)
            THROW(InvalidTropModel("Invalid GGH trop model: Rx Height"));
      }
      if(elevation < 0.0) return 0.0;
      ...
      noexcept(false)
   {
      if(!valid) {
         if(!validWeather)
            THROW(InvalidTropModel("Invalid NB trop model: weather"));
         if(!validRxLatitude)
            THROW(InvalidTropModel("Invalid NB trop model: Rx Latitude"));
         if(!validRxHeight)
            THROW(InvalidTropModel("Invalid NB trop model: Rx Height"));
         if(!validDOY)
            THROW(InvalidTropModel("Invalid NB trop model: day of year"));
      }
      ...
      noexcept(false)
   {
      if(!valid)
      {
         if(!validLat)
         {
            THROW( InvalidTropModel( "Invalid Neill trop model: Rx \
                                            Latitude" ) );
         }
         ...
         if(!validDOY)
         {
            THROW( InvalidTropModel( "Invalid Neill trop model: day \
                                            of year" ) );
         }

         THROW(InvalidTropModel( "Valid flag corrupted in Neill trop \
                                        model" ) );
      }
...
//            cout << line << endl;
      ...
      noexcept(false)
   {
      if(!valid)
      {
         if(!validLat)
         {
            THROW( InvalidTropModel( "Invalid Vienna trop model: Rx \
                                            Latitude" ) );
         }
         ...
         if(!validMJD)
         {
            THROW( InvalidTropModel( "Invalid Vienna trop model: \
                                            modified julian date" ) );
         }

         THROW(InvalidTropModel( "Valid flag corrupted in Vienna trop \
                                        model" ) );
      }
