...
//============================================================================
//
//  initial desgined by Dagoberto Salazar - gAGE ( http://www.gage.es ). 2007, 2008, 2009
//
//  2014/02/20, ShoujianZhang
//  Add the new class "Coefficient" to store the coefficients for variables.
//
//  2019/08/04, ShoujianZhang
//  Add the new class "Coefficient" to store the coefficients for variables.
//  
//  2020/01/07, shjzhang
//  Add isArcIndexed for ambiguity management
//
//============================================================================
...
namespace gnssSpace
{
   ...
   public:
      ...
      void setTypeValueData(typeValueMap& tvData)
      {
          typeValueData = tvData;
      };

      typeValueMap getTypeValueData()
      {
      ...
      SatelliteSystem::Systems getSatSys() const
      {
   ...
   struct Coefficient
   {
         /// Default constructor
      Coefficient()  
          : isConst(false), constCoeff(1.0)
      {};

         /// Default constructor
      Coefficient( bool forceCoef,
                   double coef )  
          : isConst(forceCoef), constCoeff(coef)
      {};


      Coefficient( TypeID type )  
          : isConst(false), coeffType(type)
      {};


      void setConstCoeff(double coef)
      {
          isConst = true;
          constCoeff = coef;
      };
      ...
      {
         return ( ( constCoeff == right.constCoeff )  &&
