...
/**
 * @file utilSpace.hpp
 * utilSpace namespace and GPSTK string utility functions
 */
 ...
 * utilSpace::StringException on an error. Any
 * std::exception is converted to a
 * utilSpace::StringException so that's the only
...
/** @example utilSpace_T.cpp
 * Some tests for utilSpace which may also serve as example code.
 */
...
// ################################################
         ...
         noexcept(false)
      {
        ...
        try
        {
	        ...
	        while (std::regex_search (rv,m,reg))
	        {
		        ...
		        mac = utilSpace::replaceAll(mac, rep.substr(0,1), rep.substr(1));
         ...
         noexcept(false)
      {
         try
         {
            if ((firstWord == 0) && (numWords == 1))
               return utilSpace::firstWord(s, delimiter);
            if (numWords == 0)
               return "";
                                            ...
                                            bool ignoreEmpty)
      {
         ...
         while (std::string::npos != pos || std::string::npos != lastPos)
         {
            ...
            if(trimWhitespace) token = utilSpace::strip(token);
                                                      ...
                                                      bool ignoreEmpty)
      {
         ...
         if(delimiter == '\'' || delimiter == '"')
            THROW(StringException("Delimiter must not be quote"));
         ...
         while(endPos != std::string::npos && endPos <= aStr.length())
         {
            ...
            if(currentDelimiter != delimiter)
               token = utilSpace::strip(token,currentDelimiter);
                                            ...
                                            bool showPlus)
      {
         ...
         str2 += utilSpace::rightJustify(
                     utilSpace::asString(utilSpace::asInt(str1)),elen,'0');

         // pad if necessary
         if(str2.length() < leng) str2 = utilSpace::rightJustify(str2,leng);
