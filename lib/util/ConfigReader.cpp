...
namespace utilSpace
{
   ...
   void ConfigReader::loadData(void)
      noexcept(false)
   {
         ...
         try
         {
            ...
            if( !(idx == std::string::npos) )
            {
               ...
               if( !(idx2 == std::string::npos) )
               {
                  ...
                  line = strip( line.substr(idx+1, idx2-idx-1) );
            ...
            if( !(idx == std::string::npos) )
            {
               ...
               variable = strip( line.substr(0, idx) );
               value = strip( line.substr(idx+1) );
               ...
               if( !(idx == std::string::npos) )
               {
                  varComment = strip(variable.substr(idx+1));
                  variable   = strip(variable.substr(0, idx));
               }
               ...
               if( !(idx == std::string::npos) )
               {
                  valueComment = strip(value.substr(idx+1));
                  value        = strip(value.substr(0, idx));
               }
   ...
   }  // End of method 'ConfigReader::loadData()'


   variableMap ConfigReader::getDefaultVariableData()
   {
       return confData["DEFAULT"];
   }
       ...
       *
       */
   string ConfigReader::getValue( string variable,
                                  string section,
                                  string defaultVal )
      noexcept(false)
   {
   ...
   }  // End of method 'ConfigReader::getValue()'
       ...
       *
       */
   bool ConfigReader::getValueAsBoolean( string variable,
                                           string section,
                                           bool   defaultVal )
      noexcept(false)
   {
   ...
   }  // End of method 'ConfigReader::getValueAsBoolean()'
       ...
       * 'variableList'.
       */
   string ConfigReader::fetchListValue( string variableList,
                                          string section, 
                                          string defaultVal )
      noexcept(false)
   {
      ...
      try
      {
            ...
            // Let's make sure that section and variable names are uppercase
         section      = upperCase(section);
         variableList = upperCase(variableList);
         ...
         string firstValue( stripFirstWord(origValue) );

            // Modify the originalValue value
         confData[section][variableList].value = strip(origValue);

            // Return the first value
         string value = strip(firstValue);
   ...
   }  // End of method 'ConfigReader::fetchListValue()'
       ...
       * 'variableList'.
       */
   bool ConfigReader::fetchListValueAsBoolean( string variableList,
                                                 string section,
                                                 bool   defaultVal )
      noexcept(false)
   {
   ...
   }  // End of method 'ConfigReader::fetchListValueAsBoolean()'
       ...
       *
       */
   string ConfigReader::getVariableDescription( string variable,
                                                  string section )
      noexcept(false)
   {
   ...
   }  // End of method 'ConfigReader::getVariableDescription()'
       ...
       *
       */
   string ConfigReader::getValueDescription( string variable,
                                               string section )
      noexcept(false)
   {
   ...
   }  // End of method 'ConfigReader::getValueDescription()'
       ...
       *
       */
   bool ConfigReader::ifExist( string variable,
                                 string section )
      noexcept(false)
   {
   ...
   }  // End of method 'ConfigReader::ifExist()'



      // Method to open AND load configuration data file.
   void ConfigReader::open(const char* fn)
   {
   ...
   }  // End of method 'ConfigReader::open()'



      // Method to get the name of each section in order.
   string ConfigReader::getEachSection(void)
   {
   ...
   }  // End of method 'ConfigReader::getEachSection()'
       ...
       */
   bool ConfigReader::checkName(string name)
   {
      ...
      try
      {
         ...
         if( isAlphaString(name.substr(0, 1)) )
         {

               // Translate all allowed characters to 'a'
            name = translate( name,"0123456789-_", "a", 'a');

               // If the result is an alphabetic string, the name is valid
            return ( isAlphaString(name) );
   ...
   }  // End of method 'ConfigReader::checkName()'



}  // End of namespace utilSpace
