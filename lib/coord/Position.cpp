...
/**
 * @file Position.cpp
 * class coordSpace::Position encapsulates 3-D positions, both geographic positions,
...
namespace coordSpace
{
      ...
      throw()
   {
      ...
      switch(system) {
         ...
         case Unknown:
             {
                 cerr << "Position::transformTo():Unknown system of the target" << endl;
                 GeometryException ge("Position::Unknown coordinate system of the target!");
                 THROW(ge);
             }
         case Geodetic:
            // --------------- Geodetic to ... ------------------------
   ...
   ostream& operator<<(ostream& s, const Position& p)
   {
      if(p.system == Position::Cartesian)
         ...
         s << " Unknown system! : " << fixed << setprecision(3) << setw(14) 
           << p[0] << " " << p[1] << " " << p[2];
